...
const searchNames = (logins) => {
    let matched =  logins.filter((val) => val.startsWith(".") || val.endsWith(".")).map((str) => { 
        id= logins.indexOf(str);
        return logins[id + 1];
    }
    );    
    return matched;    
...
console.log(searchNames([ "foo", "foo@bar.com", "bar", "bar@foo.com", ".foo", "food@bar.com", "bar.", "bar@foo.com" ]));
